...
double eps_over_mdot_pbh(double Mpbh, double z, double xe, double Teff) {
  ...
  if (X < 1) G_factor = 4./M_PI * sqrt(2./M_PI/X) *(1.+ 5.5*pow(X, 1.25));
  else       G_factor = 27./2./M_PI *(log(2.*X*0.56146 + 0.08) + 4./3.);

  return X/1836./137. * G_factor;   /* alpha[fine-struct] * TS/mp * G */
}
