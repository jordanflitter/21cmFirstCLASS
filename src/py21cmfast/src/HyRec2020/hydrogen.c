...
double rec_TLA_dxHIIdlna(REC_COSMOPARAMS *cosmo, double xe, double xHII, double nH, double H, double TM, double TR, double Fudge) {
  ...
  double RLya, alphaB_TM, alphaB_TR, four_betaB, C_factor, s, Dxe2, DalphaB;
  ...
  C_factor          = (3.*RLya + L2s_rescaled(fsR, meR))/(3.*RLya + L2s_rescaled(fsR, meR) + four_betaB);    /* Peebles' C-factor */
  ...
  return -nH*(s*(1.-xHII)*DalphaB + Dxe2*alphaB_TM)*C_factor/H + (cosmo->inj_params->ion + (1.-C_factor)*cosmo->inj_params->exclya)/H;
